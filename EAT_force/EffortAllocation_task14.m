...
%Set details for data output
    ...
    %Conversion of runLABEL (numeric) input to label (string)
    ...
    %Conversion of ID inputs to integers
...
%  magnitude) is stored in separate files 'cond_runLABEL_subjID'.
%  Condition files can be produced by running shuffle_conditions_effort.m] 
    ...
    if  strcmp(subj.runLABEL, 'training')
        ...
        % Load dummy maximum frequency value (=5.5)
        maxfreq_filename = sprintf('%s\\data\\dummy_freq_estimate', pwd);

    %Conditions for experiment run
...
for i_trial = 1:length(conditions) %condition file determines repetitions
    ...
    if i_trial == ((length(conditions)/2) + 1) 
        ...
        if strcmp(subj.runLABEL, 'training') 
            ...
            % Note that for tVNS project instruction during break was
            % different
...
output.filename = sprintf('effort_%s_%s_%s_s%s_%s', subj.studyID, subj.runLABEL, subj.subjectID, subj.sessionID, datestr(now, 'yymmdd_HHMM'));
