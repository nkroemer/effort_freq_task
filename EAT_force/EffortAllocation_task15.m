...
if do_fmri_flag == 0 || strcmp(subj.runLABEL, 'training') 
    ...
    text = ['Sie können in den einzelnen Durchgängen unterschiedliche Gewinne erhalten. Sie spielen dabei sowohl für Geld als auch für Kalorien, die Sie im Anschluss an die Aufgabe für einen Snack eintauschen können. Was die aktue1lle Belohnung ist, bleibt für einen Durchgang von 24 Sekunden konstant und wird Ihnen mit Hilfe von Bildern angezeigt.'];
    Screen('TextSize',w,32);
    Screen('TextFont',w,'Arial');
    [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, text, 'center', setup.ScrHeight/5, color.black,60, [], [], 1.2);
    [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, text_Cont, 'center', (setup.ScrHeight/5*4.7), color.black, 50, [], [], 1.2);
...
if strcmp(subj.runLABEL, 'training')
    text = ['Im Verlaufe des Experiments wird es unterschiedliche Schwierigkeitsstufen geben. Es wird also nicht immer möglich sein, den Ball die ganze Zeit vollständig über der Linie zu halten. Eine Möglichkeit damit umzugehen ist, auch während eines Durchgangs Pausen zu machen, um danach wieder stärkeren Druck ausüben zu können.'];
        ...
        text = ['Zur Erinnerung: Im Verlaufe des Experiments wird es unterschiedliche Schwierigkeitsstufen geben. Es wird also nicht immer möglich sein, den Ball die ganze Zeit vollständig über der Linie zu halten. Eine Möglichkeit damit umzugehen ist, auch während eines Durchgangs Pausen zu machen, um danach wieder stärkeren Druck ausüben zu können.'];
        Screen('TextSize',w,32);
        Screen('TextFont',w,'Arial');
        [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, text, 'center', setup.ScrHeight/5, color.black,60, [], [], 1.2);
        [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, text_Cont, 'center', (setup.ScrHeight/5*4.7), color.black, 50, [], [], 1.2);
...
% 
%         text = ['Nach jedem Durchgang werden Ihnen nacheinander zwei Fragen angezeigt:\n\n' char(39) 'Wie stark haben Sie sich in diesem Durchgang verausgabt?' char(39) ' \n ' char(39) 'Wie sehr wollten Sie die Belohnung in diesem Durchgang erhalten?' char(39) '\n\nSie kï¿½nnen zum Antworten den Regler auf einer Skala (ï¿½berhaupt nicht - sehr) verschieben. Nutzen Sie dazu bitte den linken Joystick auf dem Controller. Ihre Antwort mï¿½ssen Sie dann mit der grï¿½nen A-Taste auf dem Controller bestï¿½tigen.\nBitte beachten Sie, dass Sie fï¿½r die Antworten nur eine begrenzte Zeit zur Verfï¿½gung haben. ï¿½berlegen Sie deshalb nicht zu lange, sondern antworten Sie spontan. Es gibt dabei kein ' char(39) 'Richtig' char(39) ' oder ' char(39) 'Falsch' char(39) '.'];
%             Screen('TextSize',w,32);
%             Screen('TextFont',w,'Arial');
%             [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, text, 'center', setup.ScrHeight/5, color.black,60, [], [], 1.2);
%             [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, text_Cont, 'center', (setup.ScrHeight/5*4.7), color.black, 50, [], [], 1.2);
   ...
   % Draw graphical display
    ...
    % Track Ball position and translate into payout
     ...
     if ForceMat < restforce
        ...
        % -> change color, start increasing scoreï¿½pw
...
output.values_per_trial = [output.values_per_trial, [ones(1,length(ForceTime)) * subj.num ; ...  %Subj_ID
                           ...
                           t_vector ; ...                                                        %time referenced to trial start
                           t_step_vect; ...                                                      %Exact time stamp
...
output.values_per_trial = [output.values_per_trial, [ones(1,length(ForceTime)) * subj.num ; ...  %Subj_ID
                           ...
                           ones(1,length(ForceTime)) * conditions(i_trial,2); ...                %Cond.incentive 1= Money, 2= Food
                           ones(1,length(ForceTime)) * conditions(i_trial,3);   ...              %Cond.value 1 / 10 per Sec
                           zeros(1,length(ForceTime)) ...                                        %uncertainty (all 0 in training)
                           ones(1,length(ForceTime)) * output.payout_per_trial(1,i_trial); ...   %payout: Money
                           ones(1,length(ForceTime)) * output.payout_per_trial(2,i_trial)]]; ...   %payout: Food
