...
%    trial.question = 'wanted';
    ...
    if  strcmp(trial.question,'wanted')
        text_question = 'Wie sehr wolltest du die Belohnung erhalten?';
%    else text_question = ['How ' text_freerating ' is the flavor?'];
    ...
    DrawFormattedText(rating_scr, text_question, 'center', (Scale_offset+setup.ScrHeight/2 - 100), color.scale_anchors,40,[],[],2);
    DrawFormattedText(rating_scr, [anchor_1 text_freerating], (setup.ScrWidth/2-Scale_width/2 - 65), (Scale_offset+setup.ScrHeight/2 + 30), color.scale_anchors,80);
    %DrawFormattedText(rating_scr, [anchor_4], 'center', (Scale_offset+wh/2 + 30), [205 201 201],80);
    DrawFormattedText(rating_scr, [anchor_7 text_freerating], (setup.ScrWidth/2+Scale_width/2 - 75), (Scale_offset+setup.ScrHeight/2 + 30), color.scale_anchors,80);
...
%                 end
                ...
                elseif (proj_x ~= Slider_x_pos) %Update screen if participant has scrolled up or down
                    ...
                    Screen('DrawLine',w,[250 0 0],Slider_x_pos, (Scale_offset + setup.ScrWidth / 2 - 10), Slider_x_pos, (Scale_offset + setup.ScrWidth / 2 + 10),5)
